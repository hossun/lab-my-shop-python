...
bp = Blueprint('inventory', __name__, url_prefix='/inventories')

@bp.route("", methods=["POST"])
def Post():
    response = InventoryRepository.Create(request)
...
def Get():
    response = InventoryRepository.Read(request)
...
def Get_By_Id(id: int):
    response = InventoryRepository.Read_by_id(id)
...
def Update(id: int):
    ...
    response = InventoryRepository.Update(id, request)
...
def Delete(id: int):
    response = InventoryRepository.Delete(id)
    if response == None:
